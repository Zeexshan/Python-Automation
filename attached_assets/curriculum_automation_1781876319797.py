...
async def navigate_to_cell(page, cell_address):
    """Navigate to a cell using Ctrl+G (Go to range) keyboard shortcut."""
    # FIX 6: Use Ctrl+G shortcut — reliable across all Google Sheets versions
    await page.keyboard.press("Escape")
    await asyncio.sleep(0.3)
...
async def write_cell(page, cell_address, value):
    """Navigate to a cell and type a value."""
    await navigate_to_cell(page, cell_address)
    await page.keyboard.type(str(value))
    await page.keyboard.press("Tab")  # Tab confirms without jumping rows
